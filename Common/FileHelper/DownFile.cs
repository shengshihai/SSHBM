...
namespace Common
{
    ...
    public partial class DownFile
    {
        ...
        public static void CreateStaticByWebClient(string VirtualRequestUrl, string SaveVirtualPath)
        {
            ...
            string FilePath = "";
            if (VirtualRequestUrl.ToLower().Contains("http:"))
                FilePath = VirtualRequestUrl;
            else
             FilePath = HttpContext.Current.Request.Url.Scheme + "://" + HttpContext.Current.Request.Url.Authority + AppVirtualPath + VirtualRequestUrl;
