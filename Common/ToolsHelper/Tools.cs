...
namespace Common
{
    ...
    public static class Tools
    {
        ...
        public static string ToStringByList(List<int> ids, char spit)
        {
            string str = "";
            ids.ForEach(id =>
            {
        ...
        public static List<int> ToListByString(string ids, char spit)
        {
            if (string.IsNullOrEmpty(ids))
                return new List<int>();
            string[] strids = ids.Split(spit);
            List<int> listIds = new List<int>();
            foreach (var item in strids)
            {
                var id = TypeParser.ToInt32(item);
                if (id != 0)
                    listIds.Add(id);
            }
            return listIds;
