...
namespace Common
{
    ...
    public class PaginationHelper
    {
        ...
        public PaginationHelper(string tableName, string innerJoin, string selectFields, string orderName, int pageSize, int pageNumber, string orderType, string where)
        {
            ...
            if (orderName.LastIndexOf('.') == orderName.Length - 1)
            {
                this.OrderName = "";
                this.OrderType = "";
            }
        ...
        /// </summary>
        /// <returns></returns>
        public string GetSelectTopByMaxOrMinPaginationHQL()
        {
            ...
            if (!string.IsNullOrEmpty(WhereCondition))
                sbSql.AppendFormat("select  {0} from {1} {2} where {3} {4}",  SelectFields, TableName, InnerJoin, WhereCondition, strOrder);
            else
                sbSql.AppendFormat("select  {0}  from {1} {2} {3}",  SelectFields, TableName, InnerJoin, strOrder);
            ...
            //    //查询条件判断
            //    if (!string.IsNullOrEmpty(WhereCondition))
            //        sbSql.AppendFormat("select  {0} {1} from {2} {3} where {4} {5}", "", SelectFields, TableName, InnerJoin, WhereCondition, strOrder);
            //    else
            //        sbSql.AppendFormat("select  {0} {1} from {2} {3}  {4}", "", SelectFields, TableName, InnerJoin, strOrder);
            //}
            //else
            //{
            ...
            //    if (!string.IsNullOrEmpty(WhereCondition))
            //    {
            //        sbSql.AppendFormat("select  {0} {1} from {2} {3} where {4} {5} ({4}) from ( select top {6} {4} from {2}  {3} {7} ) as tblTmp)  and {8} {7} ", "", SelectFields, TableName, InnerJoin, OrderName, strTemp, (PageNumber - 1) * PageSize, strOrder, WhereCondition);
            //    }
            //    else
            //        sbSql.AppendFormat("select  {0} {1} from {2} {3} where {4} {5} ({4}) from (select top {6} {4} from {2}  {3} {7} ) as tblTmp) {7}", "", SelectFields, TableName, InnerJoin, OrderName, strTemp, (PageNumber - 1) * PageSize, strOrder);
            //}
            //return sbSql.ToString();
        }
        #endregion
